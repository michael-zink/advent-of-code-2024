...
func main() {
	...
	invalidLines := part1(input, rules)
	part2(invalidLines, rules)
}

func part1(input []string, rulesSlice []string) []string {
	result := 0
	invalidLines := []string{}
	...
	for _, line := range input {
		...
		for index, number := range numbers {
			requiredPredecessors, found := rules[number]
			if found {
				checkNumbers := numbers[:index]

				for checkIndex, checkNumber := range checkNumbers {
					if !slices.Contains(requiredPredecessors, checkNumber) {
						invalid = true
						break
					}
					if checkIndex >= index {
			...
			if !invalid {
				forbiddenSuccessors, found := rules[number]
				if found {
					checkNumbers := numbers[index+1:]
					for _, checkNumber := range checkNumbers {
						if slices.Contains(forbiddenSuccessors, checkNumber) {
							invalid = true
							break
						}
					}
	...
	println(fmt.Sprintf("Part 1: %d", result))
	return invalidLines
}

func part2(input []string, rulesSlice []string) {
	result := 0
	...
	for _, line := range input {
		...
		sort.Slice(numbers, func(i, j int) bool {
			rulesI, found := rules[numbers[i]]
			iRequiresJPredecessor := false
			jRequiresIPredecessor := false
			if found {
				if slices.Contains(rulesI, numbers[j]) {
					iRequiresJPredecessor = true
				}
			}
			rulesJ, found := rules[numbers[j]]
			if found {
				if slices.Contains(rulesJ, numbers[i]) {
					jRequiresIPredecessor = true
				}
			}

			if iRequiresJPredecessor && jRequiresIPredecessor {
				panic("Invalid input")
			}

			return jRequiresIPredecessor || !iRequiresJPredecessor
		})
	...
	println(fmt.Sprintf("Part 2: %d", result))
}
