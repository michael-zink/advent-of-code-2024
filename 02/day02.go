...
func part1(input [][]int) {
	...
	print(fmt.Sprintf("Part 1: %d\n", result))
}

func part2(input [][]int) {
	result := 0
	for _, row := range input {
		isValid := isValidRow(row, false)

		length := len(row)
		for i := 0; !isValid && i < length; i++ {
			withRemoved := removeAtIndex(row, i)
			isValid = isValidRow(withRemoved, true)
		}
	...
	print(fmt.Sprintf("Part 2: %d\n", result))
}

func isValidRow(row []int, skipEarly bool) (isValid bool) {
	decreasing := row[0] > row[1]
	skipRow := false
	for i := 0; (!skipEarly || !skipRow) && i < len(row)-1; i++ {
		if decreasing && (row[i] <= row[i+1] || row[i] > row[i+1]+3) {
			skipRow = true
		} else if !decreasing && (row[i] >= row[i+1] || row[i] < row[i+1]-3) {
			skipRow = true
		}
	}
